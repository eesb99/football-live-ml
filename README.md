...
On Streamlit Community Cloud, public viewers can refresh the cached odds when
this app secret is configured in Streamlit settings:

```toml
SPORTMONKS_API_TOKEN = "..."
```

The Paper Trading tab shows **Refresh cached odds** to public viewers. The
button writes sanitized odds snapshots to the Streamlit runtime filesystem and
clears the dashboard cache after a successful refresh. This storage is
ephemeral and can be lost on app reboot/redeploy. The refresh can spend
SportMonks provider quota, so it is capped to the next 20 fixtures and
app-wide cooldown limited to 15 minutes.
...
- The deployed public odds refresh can spend SportMonks quota, so keep the fixture cap and app-wide cooldown in place unless you intentionally change the public quota policy.
